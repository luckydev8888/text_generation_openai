    ...
    function ajax_uploadfile(form_data) {
        return $.ajax({
            ...
            success: function(response) {
                ...
                $('#hole_pdf_viewer').attr('src', 'api/pdf/' + response_file['file_path']);
            },
            error: function(xhr, status, error) {
                console.error('Error occur:', status, error);
            },
        ...
        return $.ajax({
            ...
            success: function(response) {
                ...
                for(i = 0; i < list.length; i++){
                    ...
                    txt += '<td><a href="' + list[i].url + '">' + list[i].url + '</a></td>';
                    txt += '</tr>'
                }
    ...
    function ajax_resultados() {
        return $.ajax({
            ...
            success: function(response) {
                ...
                console.log(response)
                $('#resultados_summernote')[0].innerHTML = marked.parse(response.message)
